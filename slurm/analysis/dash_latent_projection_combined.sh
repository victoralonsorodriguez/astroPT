...
#SBATCH --job-name=Latent_Combined
...
#--- LOGS FILES ---#
#SBATCH --output=logs/astropt_latent_combined_%j.out
#SBATCH --error=logs/astropt_latent_combined_%j.err
...
PYTHON_SCRIPT="$REPO_ROOT/scripts/analysis/dashboards/dash_latent_projection_combined.py"
...
echo "-----------------------------------------------"
echo "Starting Combined Latent Projections Job $SLURM_JOB_ID - $NOW"
echo "-----------------------------------------------"
...
# Note: Use the filtered catalog if available
if [ -f "/home/valonso/iac18_aasensio_shared/euclid_dr1/catalog/catalog_MER_DR1_DESI_DR1_combined_wide_deep_v1.1_FILTERED.fits" ]; then
    ...
    echo "    METADATA (Auto-filtered): $META_PATH"
fi
