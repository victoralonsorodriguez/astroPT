"""
AstroPT Multimodal Latent Projections (UMAP) - Combined Dashboard.

This script computes UMAP projections ONCE per modality and generates comparative
visualizations comparing Euclid, DESI, and Joint representations row-by-row.
No separation is made between DESI surveys; all aligned data points are plotted together.
...
logger = logging.getLogger("AstroPT-Latent-Combined")
...
def parse_args() -> argparse.Namespace:
    ...
    parser = argparse.ArgumentParser(description="AstroPT Multimodal Latent Projections (Combined)")
...
def load_data(emb_dir: Path, metadata_path: Path) -> Tuple[Dict[str, np.ndarray], pd.DataFrame]:
    """Loads embeddings and metadata with dynamic joint representation computing."""
    ...
    # Load extraction config to find joint mode
    joint_mode = "mean"
    joint_alpha = 0.5
    config_path = emb_dir / "extraction_config.json"
    if config_path.exists():
        try:
            with open(config_path, 'r') as f:
                cfg = json.load(f)
                joint_mode = cfg.get("joint_mode", "mean")
                joint_alpha = cfg.get("joint_alpha", 0.5)
                logger.info(f"[CONFIG] Loaded joint config: mode={joint_mode}, alpha={joint_alpha}")
        except Exception as e:
            logger.warning(f"Could not load extraction_config.json: {e}")

    # Dynamically compute joint embeddings if missing
    for phase_suffix in ["_phase1", "_phase2", ""]:
        euclid_key = f"EuclidImage{phase_suffix}"
        desi_key = f"DESISpectrum{phase_suffix}"
        joint_key = f"joint{phase_suffix}" if phase_suffix else "joint"
        
        if joint_key not in data_dict:
            if euclid_key in data_dict and desi_key in data_dict:
                logger.info(f"Computing joint embedding dynamically for '{joint_key}' (mode={joint_mode})")
                euclid_val = np.array(data_dict[euclid_key])
                desi_val = np.array(data_dict[desi_key])
                
                if joint_mode == "l2mean":
                    # L2-normalize vectors (rows) before averaging
                    norm_euclid = euclid_val / np.linalg.norm(euclid_val, axis=1, keepdims=True).clip(min=1e-12)
                    norm_desi = desi_val / np.linalg.norm(desi_val, axis=1, keepdims=True).clip(min=1e-12)
                    data_dict[joint_key] = 0.5 * norm_euclid + 0.5 * norm_desi
                elif joint_mode == "weighted":
                    data_dict[joint_key] = joint_alpha * euclid_val + (1.0 - joint_alpha) * desi_val
                else: # "mean"
                    data_dict[joint_key] = 0.5 * euclid_val + 0.5 * desi_val
            else:
                logger.warning(f"Could not compute '{joint_key}' because either '{euclid_key}' or '{desi_key}' is missing.")
...
def plot_combined_standard_grid(
    umap_storage: Dict[str, np.ndarray], 
    df: pd.DataFrame, 
    group_name: str, 
    group_config: dict, 
    save_dir: Path, 
    ...
    """Generates comparative grid plots with Euclid, DESI, and Joint in rows."""
    logger.info(f"Generating Comparative Grids for {group_name}...")
    ...
    row_modalities = [
        ("EuclidImage", group_config["modalities"]["EuclidImage"]),
        ("DESISpectrum", group_config["modalities"]["DESISpectrum"]),
        ("Joint", group_config["modalities"]["Joint"])
    ]
    
    def plot_single_set(set_name, plot_configs):
        logger.info(f"Generating plot set: {set_name} for {group_name}...")
        ...
        n_rows = len(row_modalities)
        n_cols = len(plot_configs)
        fig, axes = plt.subplots(n_rows, n_cols, figsize=(n_cols * 4, n_rows * 4), constrained_layout=True)
        if n_rows == 1: 
            axes = axes[np.newaxis, :] 
        
        for row_idx, (row_label, mod_key) in enumerate(row_modalities):
            if mod_key not in umap_storage:
                logger.warning(f"Modality key '{mod_key}' not found in UMAP storage. Skipping row.")
                for ax in axes[row_idx]: 
                    ax.axis('off')
                axes[row_idx, 0].set_ylabel(rf"\textbf{{{row_label}}}" + "\n" + r"\textbf{(N/A)}", fontsize=16, fontweight='bold')
                continue
                
            umap_2d = umap_storage[mod_key]
            
            # Robust limits excluding extreme outliers
            ...
            for col_idx, config in enumerate(plot_configs):
                ...
                if config["cat"]:
                    ...
                    for cat in sorted_cats:
                        cat_mask = df[col_name].astype(str) == cat
                        if np.sum(cat_mask) > 0:
                            ax.scatter(umap_2d[cat_mask, 0], umap_2d[cat_mask, 1],
                    ...
                    if config["trans"]: 
                        vals = config["trans"](vals)
                    vmin, vmax = global_limits.get(col_name, (0, 1))
                    sc = ax.scatter(umap_2d[:, 0], umap_2d[:, 1], c=vals, cmap='turbo', s=1, alpha=0.4, vmin=vmin, vmax=vmax, rasterized=True)
                    if row_idx == n_rows - 1:
                ...
                ax.set_yticks([])
                if col_idx == 0: 
                    ax.set_ylabel(rf"\textbf{{{row_label}}}" + "\n" + rf"\textbf{{(N={len(df)})}}", fontsize=18)
                if row_idx == 0: 
                    ax.set_title(rf"\boldmath\textbf{{{config['label']}}}", fontsize=20, pad=15)

        fig.suptitle(rf"\textbf{{AstroPT Latent Space - {group_config['title']} - {set_name}}}" + f"\n{suffix}", fontsize=24, y=1.05)
        
        # Save output using group prefix (e.g. umap_combined_phase1_morphology.png)
        output_file = save_dir / f"{group_name}_{set_name.lower()}.png"
        plt.savefig(output_file, dpi=300, bbox_inches='tight')
        plt.close()
        logger.info(f"Saved publication-quality comparative grid to: {output_file}")

    # Dispatch sets
    for s_name, s_config in CONFIGS.items():
        if target_set and s_name != target_set: 
            continue
...
def plot_mosaic_grid(umap_2d: np.ndarray, df: pd.DataFrame, ds: Any, args: argparse.Namespace, mod_name: str, mode: str, suffix: str, save_dir: Path):
    ...
    for k in occupied_cells: 
        occupied_cells[k].sort(key=lambda x: x[0])

    spec_fig, spec_ax = None, None
    if mode == 'spectral':
        spec_fig, spec_ax = plt.subplots(figsize=(1.5, 1.5), dpi=85)
        spec_fig.subplots_adjust(0,0,1,1)
    ...
    for i, rec in enumerate(tqdm(subset_ds, desc=f"Rendering {mode}")):
        ...
        try:
            if mode == 'visual':
                ...
                if any(v.size == 0 for v in [vis, h, j, y]): 
                    continue
                raw_bg = np.stack([vis, h, j, y], 0) - np.nanpercentile(np.stack([vis, h, j, y], 0), 60, (1,2), keepdims=True)
                vmax = np.percentile(np.abs(raw_bg), 99.5, (1,2), keepdims=True)
                vmax[vmax==0]=1.0
...
def main():
    ...
    if (args.plot_visual or args.plot_spectral) and not args.data_dir:
        logger.error("--data_dir required for visual/spectral plots.")
        sys.exit(1)
    ...
    if args.plot_visual or args.plot_spectral:
        ...
        if not loaded: 
            logger.error(f"No .arrow in {data_path}")
            sys.exit(1)
        class Wrapper: 
    ...
    # 1. Pre-compute UMAP coords for all modalities
    umap_storage = {}
    for mod, emb_full in raw_data.items():
        ...
        if cache_path.exists():
            ...
            logger.info("Computing UMAP mapping (this may take a few minutes)...")
            max_idx = len(emb_full)
            clamped_valid_idx = valid_idx[valid_idx < max_idx]
            if len(clamped_valid_idx) < len(valid_idx):
                logger.warning(f"Clamped indices to {len(clamped_valid_idx)} due to embedding dimension mismatch.")
            umap_coords = umap.UMAP(**UMAP_PARAMS).fit_transform(emb_full[clamped_valid_idx])
            np.save(cache_path, umap_coords)
            logger.info(f"UMAP coordinates cached to {cache_path.name}")
        umap_storage[mod] = umap_coords

    # 2. PHASE 2A: Standard Comparative Grids (Euclid, DESI, Joint in rows)
    if args.plot_standard:
        logger.info("\n" + "="*40 + "\nPHASE 2A: COMBINED COMPARATIVE GRIDS\n" + "="*40)
        
        PHASE_GROUPS = {
            "umap_combined_phase1": {
                "title": "Phase 1 (Isolated Unimodal Features)",
                "modalities": {
                    "EuclidImage": "EuclidImage_phase1",
                    "DESISpectrum": "DESISpectrum_phase1",
                    "Joint": "joint_phase1"
                }
            },
            "umap_combined_phase2": {
                "title": "Phase 2 (Contrastive Aligned Features)",
                "modalities": {
                    "EuclidImage": "EuclidImage_phase2",
                    "DESISpectrum": "DESISpectrum_phase2",
                    "Joint": "joint_phase2"
                }
            },
            "umap_combined": {
                "title": "General/Combined (Pooled Sequence Features)",
                "modalities": {
                    "EuclidImage": "EuclidImage",
                    "DESISpectrum": "DESISpectrum",
                    "Joint": "joint"
                }
            }
        }
        ...
        for group_name, group_config in PHASE_GROUPS.items():
            for set_name in ALL_SETS:
                logger.info(f"\n>>> PLOTTING {group_name} FOR: {set_name} <<<")
                plot_combined_standard_grid(
                    umap_storage, 
                    df_aligned, 
                    group_name, 
                    group_config, 
    ...
    # 3. PHASE 2B: Visual Mosaics (for all individual modalities)
    if args.plot_visual:
    ...
    # 4. PHASE 2C: Spectral Mosaics (for all individual modalities)
